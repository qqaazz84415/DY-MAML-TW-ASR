...
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    local/phrase2phone.sh $dir $min_lmwt $max_lmwt $wip

    # remove SIL from transcripts
    python3 local/removeSIL.py $dir $min_lmwt $max_lmwt $wip

    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_$wip/log/score.LMWT.log \
      cat $dir/scoring_kaldi/penalty_$wip/LMWT_p.txt \| \
      compute-wer --text --mode=present \
      ark:$dir/scoring_kaldi/test_filt_p.txt  ark,p:- ">&" $dir/per_LMWT_$wip || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    for lmwt in $(seq $min_lmwt $max_lmwt); do
      ...
      grep WER $dir/per_${lmwt}_${wip} /dev/null
    done
  done | utils/best_wer.sh  >& $dir/scoring_kaldi/best_per || exit 1

  best_wer_file=$(awk '{print $NF}' $dir/scoring_kaldi/best_per)
  best_wip=$(echo $best_wer_file | awk -F_ '{print $NF}')
  best_lmwt=$(echo $best_wer_file | awk -F_ '{N=NF-1; print $N}')

  if [ -z "$best_lmwt" ]; then
    echo "$0: we could not get the details of the best WER from the file $dir/per_*.  Probably something went wrong."
    exit 1;
  fi
  ...
  if $stats; then
    ...
    $cmd $dir/scoring_kaldi/log/stats1.log \
      cat $dir/scoring_kaldi/penalty_$best_wip/${best_lmwt}_p.txt \| \
      align-text --special-symbol="'***'" ark:$dir/scoring_kaldi/test_filt_p.txt ark:- ark,t:- \|  \
    ...
    $cmd $dir/scoring_kaldi/log/wer_bootci.log \
      compute-wer-bootci --mode=present \
        ark:$dir/scoring_kaldi/test_filt_p.txt ark:$dir/scoring_kaldi/penalty_$best_wip/${best_lmwt}_p.txt \
