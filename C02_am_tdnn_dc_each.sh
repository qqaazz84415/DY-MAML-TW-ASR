...
dir_tdnn_meta=$dir_tdnn/tdnn_chain_meta3
dir_meta=$dir_data_base/train_meta3
dir_ivec_meta=$dir_source_base/feature_meta_ivector3
...
if [ $stage -le 4 ]; then
  echo "-------------------- [stage 4: Meta-Training Network Configs] --------------------"
...
if [ $stage -le 5 ]; then
  ...
  for (( pass=$resume_round; pass<$meta_round; pass=pass+1 ))
  do
    rm -rf $dir_meta
    rm -rf $dir_ivec_meta
    # ===============================> Sampling data <==================================
    ...
    slot=( $(shuf -e "${slot[@]}") )
    echo "===> Sampling meta-training data for round $pass <==="
    for ((i=0; i < ${#slot[@]}; i++))
    do
      # ===============================> Sampling meta-training set <===============================
      # ===============> Support set <===============
      echo "===> Sampling support set of ${slot[${i}]} <==="
      mkdir -p $dir_meta/$i/train
      python3 local/sumTreeArgsRand.py $st_parameter $dir_dist ${slot[$i]} $dir_data_base/train_${slot[$i]} $dir_meta/$i/train
      ...
      python3 local/sumTreeArgsRand.py $st_parameter $dir_dist ${slot[$i]} $dir_data_base/train_${slot[$i]} $dir_meta/${i}_q/train
      ...
      # ====================================> MFCC extraction <=======================================
      # ===============> Support set <===============
      echo "==========> Extracting MFCC feature of support set ${slot[${i}]} <=========="
      ...
      # ================> Query set <================
      echo "===========> Extracting MFCC feature of query set ${slot[${i}]} <==========="
    ...
    do
      ...
      steps/nnet3/chain/train.py --stage=$train_stage \
        ...
        --dir=$dir_tdnn/tdnn_chain_meta3 || exit 1;
      
      # extract the logprob from log file
      python3 local/extractlogprob.py $dir_tdnn_meta/log ${slot[$i]} $pass support $dir_log_prob
      ...
      # extract the logprob from log file
      python3 local/extractlogprob.py $dir_tdnn_meta/log ${slot[$i]} $pass query $dir_log_prob
    done
    echo "==========> IQ extraction <=========="
    ...
    if (( pass < meta_round-1 )); then
      ...
      ./D01_decoding_meta_each.sh

      echo "===> modify frame subsampling factor <==="
      > $dir_tdnn_meta/frame_subsampling_factor
      echo -n "1" > $dir_tdnn_meta/frame_subsampling_factor

      echo "===> execute X01_IQ.sh <==="
      ./X01_IQ_dc.sh -f $dir_tdnn_meta -r $pass

      echo "===> restore frame subsampling factor <==="
      > $dir_tdnn_meta/frame_subsampling_factor
      echo -n "3" > $dir_tdnn_meta/frame_subsampling_factor
