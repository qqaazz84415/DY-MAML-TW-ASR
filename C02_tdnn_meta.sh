...
do
  ...
  for ((i=0; i < ${#slot[@]}; i++))
  do
    for f in $dir_ivec_meta/$i/train/feats.scp $dir_ivec_meta/${i}_q/train/feats.scp $dir_ivec_meta/$i/scp_ark/ivector_online.scp $dir_ivec_meta/${i}_q/scp_ark/ivector_online.scp \
    $dir_target_base/am_gmm_${slot[$i]}/lda_mllt/final.mdl $dir_target_base/am_gmm_${slot[$i]}/lda_mllt_ali/ali.1.gz; do
  ...
  do
    # ===============================> Lang Topo <==================================
    silphonelist=$(cat $dir_tdnn/lang_${slot[$i]}/phones/silence.csl) || exit 1;
    nonsilphonelist=$(cat $dir_tdnn/lang_${slot[$i]}/phones/nonsilence.csl) || exit 1;

    echo "===> execute steps/nnet3/chain/gen_topo.py for ${slot[$i]} <==="
    steps/nnet3/chain/gen_topo.py $nonsilphonelist $silphonelist >$dir_tdnn/lang_${slot[$i]}/topo
    for x in $i ${i}_q
    do
      # ===============================> Lattices <==================================
      ...
      steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" \
        $dir_ivec_meta/$x/train $dir_source_base/lang_${slot[$i]} $dir_target_base/am_gmm_${slot[$i]}/lda_mllt $dir_lat_meta/$x
      ...
      steps/nnet3/chain/build_tree.sh \
        ...
        $dir_tdnn/lang_${slot[$i]} $dir_target_base/am_gmm_${slot[$i]}/lda_mllt_ali $dir_tree_meta/$x
    done
  done
