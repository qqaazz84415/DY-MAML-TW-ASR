...
if [ $# -ne 3 ]; then
  ...
  echo "  --scoring-opts <string>                  # options to local/score_cer.sh"
...
if [ $stage -le 3 ]; then
  if ! $skip_scoring ; then
    [ ! -x local/score_cer.sh ] && \
      echo "Not scoring because local/score_cer.sh does not exist or not executable." && exit 1;
    echo "score best paths"
    [ "$iter" != "final" ] && iter_opt="--iter $iter"
    local/score_cer.sh $scoring_opts --cmd "$cmd" $data $graphdir $dir
