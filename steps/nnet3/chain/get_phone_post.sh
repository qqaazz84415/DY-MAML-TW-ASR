...
if [ $stage -le 0 ]; then
  if [ ! -f $dir/tacc ] || [ $dir/tacc -ot $tree_dir/ali.1.gz ]; then
    ...
    $cmd JOB=1:$this_nj $dir/log/acc_taccs.JOB.log \
       ali-to-post "ark:gunzip -c $tree_dir/ali.JOB.gz|" ark,t:- \| \
       post-to-tacc $tree_dir/final.mdl ark,t:- $dir/tacc.JOB
...
if [ $stage -le 2 ]; then
  ...
  if $use_xent_output; then
    ...
    $cmd $gpu_queue_opts JOB=1:$nj $dir/log/get_phone_post.JOB.log \
       ...
       "$model" "$feats" ark,t:- \| \
       transform-feats $dir/transform.mat ark,t:- ark,t:- \| \
       copy-feats $compress_opts ark,t:- ark,scp,t:$dir/phone_post.JOB.ark,$dir/phone_post.JOB.scp
  else
    ...
    $cmd $gpu_queue_opts JOB=1:$nj $dir/log/get_phone_post.JOB.log \
       nnet3-chain-compute-post $gpu_opt $ivector_opts --transform-mat=$dir/transform.mat \
        ...
        $model_dir/final.mdl $model_dir/den.fst "$feats" ark,t:- \| \
       copy-feats $compress_opts ark,t:- ark,scp,t:$dir/phone_post.JOB.ark,$dir/phone_post.JOB.scp
  fi
